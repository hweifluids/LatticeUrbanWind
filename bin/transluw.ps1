...
$target = Join-Path $env:LUW_HOME "core\tools_core\transluw.py"
...
if (-not $ConfPath -or $ConfPath.Trim() -eq "") {
    ...
    if (Test-Path $candidate) {
        ...
        if ($cands.Count -eq 1) {
            ...
            Write-Error "Usage: transluw <path-to-conf.luw>. For auto mode, no conf.luw in current directory and no unique *.luw found."
            exit 2
        }
    }
}
